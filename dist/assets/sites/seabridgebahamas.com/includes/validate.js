...
function Validate()
	{
		...
		else
		{
			if(document.form1.country.value == "US")
			{
				...
				{
					if(document.form1.province.value.replace(/^\s*/, '').replace(/\s*$/, '') == "")
					{
		...
		if(document.form1.amenities[4].checked == true && document.form1.otherDetail.value.replace(/^\s*/, '').replace(/\s*$/, '') == "")
		{
			alert('Please enter your favorite island amenity');
			document.form1.otherDetail.focus();
			return false;
		}
